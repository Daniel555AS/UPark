...
public class Login extends JFrame implements Runnable {
	...
	public Login() {
		...
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // Set the operation that will be performed when the window is
														// closed
		...
		/*
		 * Creation of the field where the data corresponding to the user can be
		 * entered: UPB User --> ID. Visitor --> Document No.
		 */
		...
		RoundedButton accederButton = new RoundedButton("ACCEDER", new Color(255, 239, 91), new Color(247, 208, 57),
				1000);
		...
		accederButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (text.equals("") || text.length() == 0) {
					...
					JOptionPane.showMessageDialog(null, "Debe ingresar su número ID.", "ERROR - Campo Vacío",
							JOptionPane.ERROR_MESSAGE);
					textField.setText("");
				} else {
					String[] idNumberDataFromUsuarios = databaseManager.getIdNumberDataFromUsuarios();
					String[] idNumberDataFromUsuariosActuales = databaseManager
							.getCurrentUserDataFromUsuariosActuales();
					...
					if (counterCurrentUsers > 0) {
						...
						if (counter > 0) {
							if (rolDataFromUsuarios[positionNumber].equals("Vigilante")) {
								...
								if (databaseManager.countRowsFromUsuariosActuales() == 15) {
									...
									JOptionPane.showMessageDialog(null,
											"Los cupos del parqueadero han alcanzado su límite.", "ERROR-CUPOS-LÍMITE",
											JOptionPane.ERROR_MESSAGE);
									textField.setText("");
								} else {
									soundPlayer.playSound("Media\\AccessSound.wav");
							...
							soundPlayer.playSound("Media\\ErrorSound.wav");
							JOptionPane.showMessageDialog(null, "El usuario no ha sido encontrado.", "ERROR",
									JOptionPane.ERROR_MESSAGE);
							textField.setText("");
						}
					}
		...
		RoundedButton observarMapaButton = new RoundedButton("OBSERVAR MAPA", new Color(255, 255, 255),
				new Color(196, 193, 186), 1000);
